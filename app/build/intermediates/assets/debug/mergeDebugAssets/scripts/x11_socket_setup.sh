#!/bin/sh
# ==============================================================================
# Linex X11 Socket Configuration & Validation Helper
...
echo "[Linex:X11Setup] Initializing X11 socket environment on display :${DISPLAY_NUM}..."
...
# Remove stale locks
if [ -f "$LOCK_FILE" ]; then
    echo "[Linex:X11Setup] Removing stale X11 lock: $LOCK_FILE"
    rm -f "$LOCK_FILE"
fi

if [ -S "$SOCKET_FILE" ] || [ -f "$SOCKET_FILE" ]; then
    echo "[Linex:X11Setup] Purging dangling socket: $SOCKET_FILE"
    rm -f "$SOCKET_FILE"
fi

echo "[Linex:X11Setup] X11 socket directory ready at: $X11_DIR"
exit 0
