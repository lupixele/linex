#!/bin/sh
# ==============================================================================
# LinuxDroid First Boot Setup Script
...
# and write the `.linuxdroid_initialized` marker.
# ==============================================================================
...
if [ -z "$ROOTFS" ] || [ "$ROOTFS" = "/" ]; then
    ...
    echo "[LinuxDroid:FirstBoot] ERROR: Valid rootfs directory path required: $ROOTFS"
    exit 1
fi

echo "[LinuxDroid:FirstBoot] Applying first-boot customizations to $ROOTFS..."
...
cat << 'EOF' > "$ROOTFS/etc/dpkg/dpkg.cfg.d/01_linuxdroid_nodoc"
...
cat << 'EOF' > "$ROOTFS/etc/apt/apt.conf.d/99linuxdroid"
...
CONFIG_SOURCE="/linuxdroid/config"
if [ ! -d "$CONFIG_SOURCE" ]; then
    SCRIPT_DIR="$(cd "$(dirname "$0")" 2>/dev/null && pwd || dirname "$0")"
    if [ -n "$ROOTFS" ] && [ -d "$ROOTFS/linuxdroid/config" ]; then
        CONFIG_SOURCE="$ROOTFS/linuxdroid/config"
...
if [ -f "$CONFIG_SOURCE/resolv.conf" ]; then
    echo "[LinuxDroid:FirstBoot] Copying resolv.conf from $CONFIG_SOURCE/resolv.conf..."
    cp -f "$CONFIG_SOURCE/resolv.conf" "$ROOTFS/etc/resolv.conf"
else
    echo "[LinuxDroid:FirstBoot] Falling back to builtin resolv.conf..."
...
if [ -f "$CONFIG_SOURCE/hosts" ]; then
    echo "[LinuxDroid:FirstBoot] Copying hosts from $CONFIG_SOURCE/hosts..."
    cp -f "$CONFIG_SOURCE/hosts" "$ROOTFS/etc/hosts"
else
    echo "[LinuxDroid:FirstBoot] Falling back to builtin hosts..."
...
if [ -f "$CONFIG_SOURCE/asound.conf" ]; then
    echo "[LinuxDroid:FirstBoot] Copying asound.conf from $CONFIG_SOURCE/asound.conf..."
...
cat << 'EOF' > "$ROOTFS/etc/profile.d/linuxdroid.sh"
# LinuxDroid Container Environment Profile
...
cat << EOF > "$ROOTFS/.linuxdroid_initialized"
VERSION=1.0.0
TIMESTAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ" 2>/dev/null || echo "INIT")
STATUS=READY
EOF

echo "[LinuxDroid:FirstBoot] First boot configuration successfully completed."
